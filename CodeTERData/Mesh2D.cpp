  ...
  while (!mesh_file.eof())
  {
    ...
    else if (file_line.find("Edges") != std::string::npos)
    {
      ...
      for (int i = 0 ; i < nb_edges ; ++i)
      {
        ...
        if ((ref >= verif)and(ref<=4))
        {
          _edges_for_bc[ref-1].push_back(n1);
          if (ref>verif)
          {
            _edges_for_bc[ref-2].push_back(n1);
            verif = ref;
          }
        }
      }
      _edges_for_bc[ref-1].push_back(n2);
      /*cout << "-------------------------------------------------" << endl;
      cout << "_edges_for_bc =" << endl;
      for (int i=0; i<4; i++)
      {
        for (int j = 0; j < _edges_for_bc[i].size(); j++) cout << _edges_for_bc[i][j] << " ";
        cout << endl;
      }
      cout << "-------------------------------------------------" << endl;*/
    }
